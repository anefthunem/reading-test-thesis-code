...
num_cohort5_23 <- ncol(grade5.23) - sum(colnames(grade5.23) == "kjonn")
#Print the results
print(num_cohort5_23)

#Find the number of cohort items (all columns in new dataset excluding "kjonn") - grade 8
num_cohort8_23 <- ncol(grade8.23) - sum(colnames(grade8.23) == "kjonn")
#Print the results
print(num_cohort8_23)
...
mirtest8Dgrade5.23 <- bfactor(grade5.23, specgrade5.23, bifgrade5.23, itemtype = "gpcm", quadpts = 25, TOL = 1e-4, technical = list(NCYCLES = 15000))
mirtest8Dgrade5.23
...
mirtest8Dgrade5.23SE <- bfactor(grade5.23, specgrade5.23, bifgrade5.23, itemtype = "gpcm", quadpts = 25, TOL = 1e-4, 
                                pars = mod2values(mirtest8Dgrade5.23), SE = TRUE, technical = list(NCYCLES = 15000))
...
#Estimate bifactor model with dimension-reduction method with the bfactor() function
mirtest8Dgrade8.23 <- bfactor(grade8.23, specgrade8.23, bifgrade8.23, itemtype = "gpcm", quadpts = 25, TOL = 1e-4, technical = list(NCYCLES = 15000))
mirtest8Dgrade8.23

#Estimate bifactor model with standard errors, with starting values from the estimated model above
mirtest8Dgrade8.23SE <- bfactor(grade8.23, specgrade8.23, bifgrade8.23, itemtype = "gpcm", quadpts = 25, TOL = 1e-4, 
                                pars = mod2values(mirtest8Dgrade8.23), SE = TRUE, technical = list(NCYCLES = 15000))
